...
export class TextAnalyzer {
  ...
  onInput(event: Event) {
    ...
    this.myWordsRef.nativeElement.textContent = this.text.split(/\w+/).length - 1;
  ...
  selectText() {
    ...
    this.selectedText.value = this.myTextareaRef.nativeElement.value.substring(
      this.myTextareaRef.nativeElement.selectionStart,
      this.myTextareaRef.nativeElement.selectionEnd
    ).trim();

    this.selectedText.indexStart = this.myTextareaRef.nativeElement.selectionStart
